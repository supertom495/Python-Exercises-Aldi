...
def parse_script(raw):
    get_variable = re.findall(r'\[(.*?)\]', raw)

    products = re.findall(r'\{(.*?)\}', get_variable[1])
        
    products_str = '{'
    for i, item in enumerate(products):
        products_str = products_str + '\"item' + str(i) + '\": {' + item + '}'
        # do not add ',' if it is the last item
        if i != len(products) - 1: 
            products_str += ','
...
product_storage_parsed = parse_script(product_storage_string_raw)
